...
from aipysdk import (async_openai_stream, create_tool_calls_message,
                     create_tool_result_message, human_readable_format)
...
async def chat(messages: List[ChatCompletionMessageParam]) -> AsyncStream[ChatCompletionChunk]:
...
async def main():
    async for chunk in async_openai_stream(
        stream=await chat(messages), execute_tool_calls_cb=execute_tool_calls, format=human_readable_format
    ):
